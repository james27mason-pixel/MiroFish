...
class PlatformActionLogger:
    ...
    def log_action(self, round_num: int, agent_id: int, agent_name: str,
        ...
        self._write({"round": round_num, "timestamp": datetime.now().isoformat(),
                     "agent_id": agent_id, "agent_name": agent_name,
                     "action_type": action_type, "action_args": action_args or {},
                     "result": result, "success": success})

    def log_round_start(self, round_num: int, simulated_hour: int):
        self._write({"round": round_num, "timestamp": datetime.now().isoformat(),
                     "event_type": "round_start", "simulated_hour": simulated_hour})

    def log_round_end(self, round_num: int, actions_count: int):
        self._write({"round": round_num, "timestamp": datetime.now().isoformat(),
                     "event_type": "round_end", "actions_count": actions_count})

    def log_simulation_start(self, config: Dict[str, Any]):
        self._write({"timestamp": datetime.now().isoformat(), "event_type": "simulation_start",
    ...
    def log_simulation_end(self, total_rounds: int, total_actions: int):
        self._write({"timestamp": datetime.now().isoformat(), "event_type": "simulation_end",
                     "platform": self.platform, "total_rounds": total_rounds,
...
class SimulationLogManager:
    ...
    SimulationRunner already owns simulation.log and redirects the child process'
    stdout/stderr to it.  Opening that same path here (especially with mode='w')
    creates competing file offsets and can truncate/hide the diagnostics we need.
    Therefore the child logger writes to stdout only; the parent captures stdout
    into simulation.log exactly once.
    """
...
class ActionLogger:
    ...
    def log_action(self, round_num, platform, agent_id, agent_name, action_type,
                   action_args=None, result=None, success=True):
        self._write({"round": round_num, "timestamp": datetime.now().isoformat(), "platform": platform,
                     "agent_id": agent_id, "agent_name": agent_name, "action_type": action_type,
                     "action_args": action_args or {}, "result": result, "success": success})

    def log_round_start(self, round_num, simulated_hour, platform):
        self._write({"round": round_num, "timestamp": datetime.now().isoformat(), "platform": platform,
                     "event_type": "round_start", "simulated_hour": simulated_hour})

    def log_round_end(self, round_num, actions_count, platform):
        self._write({"round": round_num, "timestamp": datetime.now().isoformat(), "platform": platform,
                     "event_type": "round_end", "actions_count": actions_count})

    def log_simulation_start(self, platform, config):
        self._write({"timestamp": datetime.now().isoformat(), "platform": platform,
                     "event_type": "simulation_start",
                     "total_rounds": config.get("time_config", {}).get("total_simulation_hours", 72) * 2,
                     "agents_count": len(config.get("agent_configs", []))})

    def log_simulation_end(self, platform, total_rounds, total_actions):
        self._write({"timestamp": datetime.now().isoformat(), "platform": platform,
                     "event_type": "simulation_end", "total_rounds": total_rounds,
                     "total_actions": total_actions})
